    ...
    <div class="col-md-4 col-md-offset-4">
        <div class="panel panel-default">
            <div class="panel-heading">
                @if(isset($target))
                    <span class="glyphicon glyphicon-warning-sign"></span> {{ $target['text'] }}
                @endif
            </div>
            <div class="panel-body">
                @if(isset($records))
                    <div class="input-group input-group-sm">
                        ...
                        <input type="hidden" id="type" class="form-control" value="{{ $target['key'] }}">
                        <span class="input-group-btn">
                            <button class="btn btn-info" type="button" data-toggle="modal" data-target="#myModal">+ 新项目</button>
                        </span>
                    </div><!-- /input-group -->
                    ...
                    <table class="table">
                        ...
                        <tr>
                            <td><a href="/config/set/{{ $record->type.'-'.$record->id }}" class="btn btn-xs btn-{{ $record->show ? 'success' : 'warning' }}">{{ $record->text }}</a></td>
...
<div class="modal fade" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <script>
        function send() {
            ...
            var post_data = {'type': type_val, 'text': text_input};
            var post_url = '/config/add/post';
